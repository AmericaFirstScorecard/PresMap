import { getStateFP, statesGeoJSON, countiesGeoJSON, countyElectionStats, stateStats, updateGeoJSONAndMap } from './data.js';
...
function initializeMap() {
  ...
  mainMap.on('load', () => {
    if (!statesGeoJSON || !statesGeoJSON.features) {
      ...
      countiesGeoJSON.features.forEach(f => {
        ...
        if (!stateFeatures[stateFP]) {
          stateFeatures[stateFP] = {
            type: "Feature",
            properties: { STATEFP: stateFP, name: Object.keys(window.stateFipsMap).find(key => window.stateFipsMap[key] === stateFP) || `State ${stateFP}` },
      ...
      window.statesGeoJSON = { type: "FeatureCollection", features: Object.values(stateFeatures) };
    }
...
export { initializeMap, addCountiesLayer, removeCountiesLayer, mainMap, currentState };
