...
function populateCountyStatsFromCSV(rows, headers) {
...
export function getStateFP(stateName) {
...
export function updateGeoJSONAndMap() {
  ...
  if (window.mainMap && window.mainMap.getSource('states')) {
    window.mainMap.getSource('states').setData(statesGeoJSON);
    window.mainMap.triggerRepaint();
  }
  // UI updates are handled in ui.js
}

export async function refreshElectionData() {
  try {
    await Promise.all([
